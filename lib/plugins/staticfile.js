...
var mime = {
  ...
  getType: function (path) {
    ...
    var type = this.types[path.substring(index).toLowerCase()] || this.defaultType;
...
module.exports = function (opts) {
  ...
  var pathVar = opts.pathVar || 'filepath';
  return function (req, resp, chain) {
    var self = this;
    var absPath = path.join(root, this.params[pathVar].replace(/^\.*/, ''));
    fs.readFile(absPath, function (err, data) {
      if (err != null) {
        chain();
        return;
      }
      self.headers['Content-Type'] = mime.getType(absPath);
      self.headers['Content-Length'] = data.length;
      self.send(data, typeof data === 'string' ? 'utf-8' : 'binary');
    });
  };
}
